...
import biz.riman.erp.batch.mapper.salesOrderCancel.JupiterSalesOrderCancelMapper;
import biz.riman.erp.batch.parameter.SalesOrderCancelParameter;
import lombok.extern.slf4j.Slf4j;

@Slf4j
@Service
public class JupiterSalesOrderCancelService {
    @Autowired
    private JupiterSalesOrderCancelMapper mapper;
    @Autowired
    ...
    @Autowired
    public JupiterSalesOrderCancelService(@Qualifier("oAuth2WebClient") WebClient localApiClient) {
    ...
    public List<SalesOrderCancelDto> setBodyParam(SalesOrderCancelParameter parameter) throws Exception {
        ...
        if (CollectionUtils.isNotEmpty(bodyParams)) {
            for (SalesOrderCancelDto bodyParam : bodyParams) {
                ...
                // 주문취소 쿠폰 정보 setting
                SalesOrderCancelPricingDto pricing = new SalesOrderCancelPricingDto();
                BeanUtils.copyProperties(bodyParam, pricing);
                bodyParam.setTo_Pricing(new ArrayList<SalesOrderCancelPricingDto>(List.of(pricing)));
